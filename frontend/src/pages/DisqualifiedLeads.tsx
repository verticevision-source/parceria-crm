...
export default function DisqualifiedLeads() {
  ...
  async function confirmAssign() {
    ...
    setAssigning(true)
    try {
      await whatsappApi.routeToVendor(assignLead.contact.phone, vendorId, message.trim() || undefined)
      toast.success('Lead atribuído ao vendedor!')
      setAssignLead(null)
      load()
    } catch (e: any) {
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6">
      ...
      >
        ...
        <label className="text-sm text-text-muted mt-3 block">Mensagem pro cliente (opcional)</label>
        <textarea
          className="input w-full mt-1 text-sm"
          rows={3}
          placeholder="Deixe em branco pra não enviar nada agora"
          value={message}
          onChange={(e) => setMessage(e.target.value)}
        />
        <p className="text-[11px] text-text-muted mt-1">Se preenchida, é enviada pelo WhatsApp do vendedor escolhido.</p>
